...
{
  /*uint16 usRxBufLen = Hal_UART_RxBufLen(HAL_UART_PORT_0); // ¶ÁÈ¡½ÓÊÕ¾ÝÁ¿

¡¡if(usRxBufLen)
¡¡{
    usRxBufLen = MIN(128£¬usRxBufLen);

¡¡¡¡uint16 readLen = HalUARTRead(HAL_UART_PORT_0£¬ &SerialRxBuf£ÛRxIndex£Ý£¬ usRxBufLen); // ¶ÁÈ¡Êý¾Ýµ½»º³åÇø

¡¡¡¡RxIndex += readLen;

¡¡¡¡readLen %= 128;

¡¡¡¡osal_start_timerEx(simpleBLEPeripheral_TaskID, SBP_READ_ZM516X_INFO_EVT 5); // Æô¶¯¶¨Ê±Æ÷
